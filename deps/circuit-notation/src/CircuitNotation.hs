...
-- | A variable pattern carrying @loc@ on the pattern AND on the located name
-- inside it: renamer-stage tooling (e.g. tracing plugins) distinguishes
-- source binders from compiler-generated ones by the NAME's span, and
-- generated port binders should read as source-located.
varP :: SrcSpanAnnA -> String -> LPat GhcPs
varP loc nm = L loc $ VarPat noExtField (L (noAnnSrcSpan (locA loc)) $ var nm)
...
bindWithSuffix dflags = bindWithSuffixNamed dflags portBindName

-- | The name a leaf port binds under: @\<port\>_\<direction\>@.
portBindName :: PortName -> Direction -> String
portBindName (PortName _ fs) dir = GHC.unpackFS fs <> "_" <> show dir
...
bindWithSuffixNamed dflags nameOf dir = \case
  p@(Tuple ps) -> let loc = noAnnSrcSpan (portDescLoc p)
    in tildeP loc $ taggedBundleP loc $ tupP loc $ fmap (bindWithSuffixNamed dflags nameOf dir) ps
  Vec s ps -> taggedBundleP s $ vecP s $ fmap (bindWithSuffixNamed dflags nameOf dir) ps
  Ref pn@(PortName loc _) -> varP loc (nameOf pn dir)
  RefMulticast pn@(PortName loc _) -> case dir of
    Bwd -> L loc (WildPat noExtField)
    Fwd -> varP loc (nameOf pn dir)
  ...
  Lazy loc p -> tildeP loc $ bindWithSuffixNamed dflags nameOf dir p
  ...
  PortType ty p -> tagTypeP dir ty $ bindWithSuffixNamed dflags nameOf dir p

{- | The named leaf binders of a port description in a given direction —
exactly the 'Ref'\/'RefMulticast' leaves 'bindWithSuffixNamed' turns into
'varP's, in the same order.
-}
portLeaves :: Direction -> PortDescription PortName -> [(PortName, Direction)]
portLeaves dir = \case
  Tuple ps -> concatMap (portLeaves dir) ps
  Vec _ ps -> concatMap (portLeaves dir) ps
  Ref pn -> [(pn, dir)]
  RefMulticast pn -> case dir of
    Bwd -> []
    Fwd -> [(pn, dir)]
  Lazy _ p -> portLeaves dir p
  PortType _ p -> portLeaves dir p
  _ -> []
...
bindOutputs
  ...
  -- ^ master ports
  -> LPat p
bindOutputs dflags = bindOutputsNamed dflags portBindName

-- | 'bindOutputs' with the leaf binder names picked by @nameOf@.
bindOutputsNamed
  :: (p ~ GhcPs, ?nms :: ExternalNames)
  => GHC.DynFlags
  -> (PortName -> Direction -> String)
  -> Direction
  -> PortDescription PortName
  -- ^ slave ports
  -> PortDescription PortName
  -- ^ master ports
  -> LPat p
bindOutputsNamed dflags nameOf direc slaves masters = noLoc $ conPatIn (noLoc (fwdBwdCon ?nms)) (InfixCon m2s s2m)
  where
  m2s = bindWithSuffixNamed dflags nameOf direc masters
  s2m = bindWithSuffixNamed dflags nameOf (revDirec direc) slaves

{- | @trace-ports@ support: re-bind every lambda-bound interface port half to
its user-visible name (@\<port\>_\<direction\>@) through a trivial @let@
indirection carrying the port's REAL source span. The lambda itself then
binds fresh names ('lamBindName'), so renamer-stage tooling — e.g. a tracing
plugin that instruments local bindings — sees ALL circuit ports as ordinary
source-located let binders, interface and intermediate alike. The
indirections are value-identical lazy aliases inside the circuit's existing
recursive @let@: free at runtime, inference-neutral, and safe in the
feedback knot. Direction assignment mirrors 'bindOutputsNamed' applied to
the same arguments.
-}
lamIndirections
  :: Direction
  ...
  -- ^ master ports
  -> [LHsBind GhcPs]
lamIndirections direc slaves masters =
  fmap mk (portLeaves direc masters <> portLeaves (revDirec direc) slaves)
  where
  mk (pn@(PortName loc _), dir) =
    L loc
      (patBind
        (varP loc (portBindName pn dir))
        (varE noSrcSpanA (var (lamBindName pn dir))))
...
busCircuitQQExpM
  :: (p ~ GhcPs, ?nms :: ExternalNames, ?tracePorts :: Bool)
  => CircuitM (LHsExpr p)
busCircuitQQExpM = do
  checkCircuit
  ...
  -- trace-ports: the lambda binds fresh names and every interface port half
  -- is re-bound to its user-visible name by a let indirection, so
  -- renamer-stage tooling sees interface ports as ordinary local binders
  -- (see 'lamIndirections').
  let indirs
        | ?tracePorts = lamIndirections Fwd masters slaves
        | otherwise = []
  let decs = indirs <> lets <> completes <> map decFromBinding' binds

  let pats
        | ?tracePorts = bindOutputsNamed dflags lamBindName Fwd masters slaves
        | otherwise = bindOutputs dflags Fwd masters slaves
...
valueCircuitQQExpM = do
  ...
  -- including the trace-ports interface indirections.
  let decFromBinding' b@Binding{bCircuit = L cloc _} = L cloc (decFromBinding dflags b)
  let indirs
        | ?tracePorts = lamIndirections Fwd masters1 slaves1
        | otherwise = []
  let decs = indirs <> compDecs <> outerLets <> completes <> map decFromBinding' binds2

  let pats
        | ?tracePorts = bindOutputsNamed dflags lamBindName Fwd masters1 slaves1
        | otherwise = bindOutputs dflags Fwd masters1 slaves1
...
transform
    :: (?nms :: ExternalNames, ?tracePorts :: Bool)
    => Bool
...
pluginImpl cliOptions _modSummary m = do
    -- cli options are activated by -fplugin-opt=CircuitNotation:<option>;
    -- "debug" prints the generated code, "trace-ports" re-binds
    -- lambda-bound interface ports through source-located let indirections
    -- (see 'lamIndirections').
    let knownOpts = ["debug", "trace-ports"]
        unknownOpts = filter (`notElem` knownOpts) cliOptions
        debug = "debug" `elem` cliOptions
        tracePorts = "trace-ports" `elem` cliOptions
    unless (null unknownOpts) $
      warningMsg $ Outputable.text $ "CircuitNotation: unknown cli options " <> show unknownOpts
    hpm_module' <- do
      let ?tracePorts = tracePorts
       in transform debug $ GHC.hpm_module $ GHC.parsedResultModule m
