...
-- Auto-instrument this module for scoped simulation tracing:
-- 'xilinxElasticBuffer' (OPAQUE + 'HasCircuitContext') becomes a VCD
-- hierarchy level whose named internal signals are traced when simulated
-- under 'Clash.CircuitContext.withCircuitContext'. Instrumentation is
-- transparent to HDL generation, so this stays synthesizable; the
-- synthesis-facing caller 'xilinxElasticBufferWb' invokes it through
-- 'withoutCircuitContext' so it need not carry the 'HasCircuitContext'
-- constraint.
...
xilinxElasticBufferWb clkRead rstRead SNat localCounter clkWrite wdata =
  withClockResetEnable clkRead rstRead enableGen $ circuit $ \wb -> do
    ...
    let
      ...
      -- Context flows into the inner buffer so its internals nest under this
      -- wrapper's VCD scope (the wrapper now carries 'HasCircuitContext'; its
      -- synthesis callers discharge it with 'withoutCircuitContext').
