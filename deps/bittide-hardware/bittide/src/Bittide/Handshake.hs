...
-- Auto-instrument this module for scoped simulation tracing: 'handshake'
-- (OPAQUE + 'HasCircuitContext') becomes a VCD hierarchy level whose internal
-- FSM signals (statuses, decoded/transmitted 'Meta', word out) are traced.
-- 'perChannel' and the 'handshakesWb' peripheral (also OPAQUE +
-- 'HasCircuitContext') propagate the constraint so a per-channel 'handshake'
-- nests under 'handshakesWb' when the design is traced; a synthesis caller
-- discharges it once at the boundary with 'withoutCircuitContext'.
-- Instrumentation is HDL-transparent.
...
perChannel ::
  (HasCircuitContext, HiddenClock dom) =>
  Signal dom Mode ->
  Signal dom Bool ->
  Signal dom Bool ->
...
perChannel mode receiveReady softwareReady fromNeighbor fromCore =
 ...
 where
  ...
  (handshakeToNeighbor, neighborToCoreStatus, coreToNeighborStatus, maybeSoftwareReady) =
    handshake handshakeReset fromNeighbor receiveReady softwareReady
