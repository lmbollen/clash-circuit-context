...
import Clash.CircuitContext (HasCircuitContext, HasProbe, mealyBProbed, mealyProbed)
...
domainDiffCounter clkSrc rstSrc clkDst rstDst =
  mealyProbed clkDst rstDst enableGen go DdcInReset counter
 where
  ...
  go :: (HasProbe) => DdcState -> Unsigned 64 -> (DdcState, (Signed 32, Bool))
  go DdcInReset c1
...
extendSuccCounter clk rst counterLower =
 ...
 where
  go :: (HasProbe) => EscState m -> (Bool, Unsigned n) -> (EscState m, Unsigned (m + n))
