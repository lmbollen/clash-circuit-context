  ...
  let
    ...
    dutMem =
      withClockResetEnable clk rst ena
        $ unMemmap
        $ withoutCircuitContext (wbStorage "test" (SNat @(2 ^ (AddressWidth - 1))) (Just (Vec (repeat 0))))
...
testIncreaseBusWidth power = property $ do
  let
    ...
    dutMem =
      withClockResetEnable clk rst ena
        $ unMemmap
        $ withoutCircuitContext (wbStorage "test" depth (Just (Vec (repeat 0))))
...
prop_memoryModel = property $ do
  let
    ...
    dut =
      unMemmap (withoutCircuitContext (wbStorage "test" d8 (Just (Vec (repeat 0)))))
