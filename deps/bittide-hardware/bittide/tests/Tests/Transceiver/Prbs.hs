  ...
  case config of
    (Prbs.Config :: Prbs.Config polyLength polyTap nBits) -> do
      ...
      let
        ...
        nSamp = okAfter + checkOk

      -- Build the generator→align→checker pipeline under a circuit context and
      -- dump a hierarchical VCD of the last run: 'generator' traces its output,
      -- 'checker' probes its mealy step internals. The pipeline signals between
...
enough good cycles; the VCD shows the @tracker@ scope with that signal.
-}
case_trackerWaveform :: Assertion
case_trackerWaveform = do
  let
    clk = clockGen @XilinxSystem
...
tests =
  testGroup
    ...
    , testCase "case_trackerWaveform" case_trackerWaveform
    ]
