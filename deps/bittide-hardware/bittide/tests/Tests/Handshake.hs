...
-- Auto-instrument this module's designs for scoped simulation tracing. The
-- OPAQUE + 'HasCircuitContext' functions ('dut', 'handshakesWithDelays')
-- become VCD hierarchy levels; their named local signals are auto-traced.
...
{- | Derived generically ('Generic' + an empty instance): a binding of this
type in an instrumented function auto-traces field-wise, each field a
@\<binder\>.\<field\>@ sub-scope wire in the VCD.
-}
instance (KnownDomain dom, KnownNat n) => Traceable (HandshakeOut dom n)
...
instance (KnownDomain dom, KnownNat n) => Traceable (HandshakesOut dom n)
