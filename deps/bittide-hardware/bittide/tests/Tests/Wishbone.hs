  ...
  let
    ...
    dutMm =
      withLittleEndian $ withClockResetEnable clk rst ena $ circuit $ \(mm, wb) -> do
        ...
        (uartOut, _uartStatus) <- withoutCircuitContext (uartInterfaceWb d2 maxInFlight uartBytes) -< ((mm, wb), uartIn)
        uartIn <-
   ...
   where
    ...
    (toMaster, toSlaves) =
      withClockResetEnable
        ...
        resetGen
        enableGen
        (withoutCircuitContext singleMasterInterconnect')
        config
        masterIn
        $ bundle slaves
...
prop_dfWishboneMaster =
 ...
 where
  ...
  impl = withClockResetEnable clockGen resetGen enableGen $ circuit $ \reqs -> do
    (wb, resps) <- withoutCircuitContext dfWishboneMaster -< reqs
