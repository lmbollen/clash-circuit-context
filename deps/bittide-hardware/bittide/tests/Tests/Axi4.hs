...
tests =
  testGroup
    ...
    , testPropertyNamed
        "prop_wishboneS2Axi4_Handshakes"
        "prop_wishboneS2Axi4_Handshakes"
        prop_wishboneS2Axi4_Handshakes
    , testCase "case_axi4StreamPacketFifoWaveform" case_axi4StreamPacketFifoWaveform
    ]

{- | Waveform for a @sampleC@-driven circuit.
...
This is a single deterministic run in its own test case — so it writes exactly
one waveform, to a file named after the case.
-}
...
case_axi4StreamPacketFifoWaveform = do
  ...
  assertBool
    "expected the input transfers to reappear on the FIFO output"
    (validTransfers samples == expectedTransfers)
 where
  conf = SimulationConfig 0 nSamples True
  nSamples = 64
  driven =
    withClockResetEnable @System clockGen noReset enableGen $
      axiStreamPacketFifo d2 d8 <| driveC conf demoInput
  validTransfers xs = [(d, l) | (d, l, valid) <- xs, valid]
  expectedTransfers =
    [(pack (_tdata t), _tlast t) | Just t <- demoInput]

-- | A concrete 4-byte AXI4-Stream transfer (no id/dest, unit user).
axiTransfer :: Vec 4 (Unsigned 8) -> Bool -> Axi4StreamM2S ('Axi4StreamConfig 4 0 0) ()
axiTransfer dat lastTransfer =
  Axi4StreamM2S
    { _tdata = dat
    , _tkeep = repeat True
    , _tstrb = repeat True
...
-- | Two deterministic packets to push through the FIFO for the waveform.
demoInput :: [Maybe (Axi4StreamM2S ('Axi4StreamConfig 4 0 0) ())]
demoInput =
  [ Just (axiTransfer (0x10 :> 0x11 :> 0x12 :> 0x13 :> Nil) False)
  , Just (axiTransfer (0x14 :> 0x15 :> 0x16 :> 0x17 :> Nil) True)
  , Just (axiTransfer (0x20 :> 0x21 :> 0x22 :> 0x23 :> Nil) True)
  ]

{- | 'BitPack'able view of a transfer to trace: packed data, @tlast@, and a
validity bit (there is no signal to trace on cycles the manager sends nothing).
-}
summarizeAxiTransfer ::
  Maybe (Axi4StreamM2S ('Axi4StreamConfig 4 0 0) ()) -> (BitVector 32, Bool, Bool)
summarizeAxiTransfer Nothing = (0, False, False)
summarizeAxiTransfer (Just t) = (pack (_tdata t), _tlast t, True)
...
prop_wbAxisRxBufferReadStreams = property $ do
 ...
 where
  ...
  rxBufNoMm bufBytes =
    let Circuit f = withLittleEndian (withoutCircuitContext (wbAxisRxBufferCircuit @System @30 bufBytes))
...
prop_wishboneS2Axi4 = property $ do
  ...
  let
    ...
    impl = withClockResetEnable clockGen resetGen enableGen $ circuit $ \wbIn -> do
      ...
      (wbOut, rd, wr) <- withoutCircuitContext axiMemoryMappedToWb -< (ra, wa, wd)
      subordinate -< wbOut
...
when connected to the `axiDummySub` component. This requires `wbToAxi4MemoryMapped`
to correctly handle all channels of the Axi4 channels and Wishbone protocols.
-}
prop_wishboneS2Axi4_Handshakes :: Property
prop_wishboneS2Axi4_Handshakes = property $ do
  ...
  requestsAndStalls <- forAll $ Gen.list (Range.linear 0 100) gen
  let
...
axiDummySub addrRange = Circuit go
 where
  go ~(~(raFwd, waFwd, wdFwd), ~(rdBwd, wrBwd)) = ((raBwd, waBwd, wdBwd), (rdFwd, wrFwd))
   where
    ~(raBwd, rdFwd) = mealyB readHandler Nothing (raFwd, rdBwd)
    ~(waBwd, wdBwd, wrFwd) = mealyB writeHandler (False, False) (waFwd, wdFwd, wrBwd)

  getAckOrErr addr =
    if addr < addrRange
      then Axi.ROkay
      else Axi.RSlaveError
  mkRdFwd resp =
    Axi.S2M_ReadData
      { _rid = 0
      , _rdata = 0
  ...
  -- Deal with a new read address
  readHandler Nothing ~(raFwd, rdBwd) = (maybeResp, (raBwd, rdFwd))
   where
    raAvailable = raFwd /= Axi.M2S_NoReadAddress
    raBwd = Axi.S2M_ReadAddress True
    resp = getAckOrErr raFwd._araddr
    rdFwd
      | raAvailable = mkRdFwd resp
      | otherwise = Axi.S2M_NoReadData
    maybeResp
      | raAvailable && rdBwd._rready = Nothing -- Immediate reply from rdBwd
      | raAvailable = Just resp -- rdBwd not ready
      | otherwise = Nothing -- No rdFwd

  -- Deal with an existing read address
  readHandler (Just resp0) ~(raFwd, rdBwd) = (maybeResp, (raBwd, rdFwd))
   where
    rdFwd = mkRdFwd resp0
    raBwd = Axi.S2M_ReadAddress $ rdBwd._rready
    raAvailable = raFwd /= Axi.M2S_NoReadAddress
    resp1 = getAckOrErr raFwd._araddr
    maybeResp
      | not rdBwd._rready = Just resp0 -- No reply from rdBwd
      | raAvailable && rdBwd._rready = Just resp1 -- Reply from rdBwd and new raFwd
      | otherwise = Nothing

  -- Deal with a new write address
  writeHandler (receivedAddr, receivedData) ~(waFwd, wdFwd, wrBwd) = (nextState, (waBwd, wdBwd, wrFwd))
   where
    waBwd = Axi.S2M_WriteAddress (not receivedAddr)
    wdBwd = Axi.S2M_WriteData (not receivedData)

    wrFwd
      | receivedAddr && receivedData =
    ...
    nextState
      | receivedAddr && receivedData && wrBwd._bready = (False, False)
      | otherwise =
          ( receivedAddr || waFwd /= Axi.M2S_NoWriteAddress
          , receivedData || wdFwd /= Axi.M2S_NoWriteData
          )

-- Utilities
isRead, isWrite :: WB.WishboneMasterRequest addrW dat -> Bool
...
findRead ::
  (KnownNat addrW) =>
  BitVector addrW ->
  [(WB.WishboneMasterRequest addrW dat, WB.WishboneS2M dat)] ->
  Bool
...
findReadAfterWrite ::
  (KnownNat addrW) => [(WB.WishboneMasterRequest addrW dat, WB.WishboneS2M dat)] -> Bool
