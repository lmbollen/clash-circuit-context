...
readWriteByteAddressableBlockram = property $ do
  ...
  case ramContents of
    BitvecVec SNat SNat contents -> do
      ...
      -- Trace by composing 'traceSignalC' onto the simulated function, so
      -- 'simulateN's reset-priming ('simulateWithResetN') is preserved —
      -- unfolding it to 'sampleN (f (fromList xs))' would drop the priming
  ...
  go depth@SNat = do
    ...
    wcre
      $ wishbonePropWithModel @System
        defExpectOptions
        (\_ _ () -> Right ())
        (unMemmap $ withoutCircuitContext (wbStorage "" depth (Just (Vec content))))
 ...
 where
  ...
  go depth@SNat = do
    ...
    let
      ...
      slave = wcre $ unMemmap (withoutCircuitContext (wbStorage @System @_ @30 "" depth (Just (Vec content))))
