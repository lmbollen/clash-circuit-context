...
elasticBufferWb clkRead rstRead clkWrite wbIn wdata = (wbOut, dataCount, underflow, overflow, readData)
 where
  ...
  ((SimOnly _mm, wbOut), (dataCount, underflow, overflow, readData)) =
    withLittleEndian
      $ toSignals
        (withoutCircuitContext (xilinxElasticBufferWb clkRead rstRead d5 localCounter clkWrite wdata))
        (((), wbIn), ((), (), (), ()))
 ...
 where
  dut (unbundle -> (margin, dataCount, ack)) =
    bundle
      $ snd
      $ toSignals
        (withLittleEndian $ withoutCircuitContext $ withClock clk $ autoCenter @_ @5 rst enableGen margin dataCount)
        ((), (ack, (), ()))
