...
vexRiscGmiiC sysClk sysRst rxClk rxRst txClk txRst peConfig =
  circuit $ \(mm, (uartTx, gmiiRx, jtag)) -> do
    ...
    idC -< (uartRx, gmiiTx)
 where
  time = withLittleEndian $ withoutCircuitContext $ wcre $ timeWb Nothing
  -- 'readDnaPortE2Wb' opts into scoped simulation tracing (a viral
  -- 'HasCircuitContext'); this synthesis-facing instance discharges it with
  -- 'withoutCircuitContext' so it stays context-free.
  dnaC = withLittleEndian $ withoutCircuitContext $ wcre readDnaPortE2Wb simDna2
  mac =
    withoutCircuitContext
      ...
      txClkEna
      rxClkEna
  macStatIf = withoutCircuitContext $ wcre $ macStatusInterfaceWb d16
  uart = withLittleEndian $ withoutCircuitContext $ wcre $ uartInterfaceWb d32 d2 (uartDf baud)
  pe = withLittleEndian $ withoutCircuitContext $ wcre processingElement NoDumpVcd peConfig
  wbToAxi4StreamTx' = withLittleEndian $ withoutCircuitContext $ wcre wbToAxi4StreamTx
  wbAxiRxBuffer = withLittleEndian $ withoutCircuitContext $ wcre wbAxisRxBufferCircuit (SNat @2048)
