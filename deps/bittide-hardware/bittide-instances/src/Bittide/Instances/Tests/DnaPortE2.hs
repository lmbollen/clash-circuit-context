...
-- Auto-instrument the DUT for scoped simulation tracing: the plugin wraps the
-- OPAQUE + 'HasCircuitContext' 'dutWithMm' in a VCD component scope, and the
-- instrumented peripherals it wires up (e.g. 'readDnaPortE2Wb') nest under it.
...
dutWithMm ::
  forall dom.
  (HasCircuitContext, HiddenClockResetEnable dom) =>
  PeConfig 4 ->
