...
dutUartStream dumpVcd peConfig = withoutCircuitContext (dutUartStreamC dumpVcd peConfig)

-- | Like 'dutUartStream', but under the caller's circuit context (so a live
-- waveform capture can record the run).
dutUartStreamC :: (HasCallStack, HasCircuitContext) => DumpVcd -> PeConfig 4 -> String
dutUartStreamC dumpVcd peConfig = chr . fromIntegral <$> catMaybes uartStream
 where
  uartStream =
...
dutWithVcdAndPeConfig ::
  (HasCallStack, HasCircuitContext) =>
  DumpVcd ->
  PeConfig 4 ->
...
memoryMap =
  getMMAny
    $ withoutCircuitContext (dutWithVcdAndPeConfig NoDumpVcd)
