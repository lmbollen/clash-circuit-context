...
deviceWithOffsetsWb ::
  ...
  (HasCallStack, HasCircuitContext, KnownDomain dom, KnownNat n, KnownNat aw, KnownNat wordSize) =>
  Clock dom ->
  Reset dom ->
  DeviceConfig ->
  Circuit
 ...
 where
  ...
  go (((offset, SimOnly config, _, wbM2S0), aIn0), (_, coerce -> dfAck)) =
   ...
   where
    ...
    -- ccc: record the register's stored contents in the hierarchical waveform on
    -- EVERY invocation -- including wrappers like 'registerWb_' that discard the
    -- value output 'aOut'. 'traceSignalC'/'registerTrace' records in lockstep with
    -- the tapped signal being *sampled*, so the tap must sit on an always-sampled
    -- path: we thread it through the bus response 'wbS2M2' (consumed every cycle by
    -- the interconnect for every register, used or not). Gated on 'clashSimulation'
    -- so synthesis sees pure 'wbS2M2' with no spurious dependency on the contents.
    -- 'registerWbDf' is the worker behind the whole register* family, so this
    -- covers registerWb/registerWb_/registerWbVec*/registerWithOffset*/*I alike.
    wbS2M2Traced
      | clashSimulation =
          (\c s -> c `seq` s)
