> ### You are on `dogfood/bittide`
>
> This branch is the plugin — everything below — **plus one vendored,
> instrumented checkout**: [`deps/bittide-hardware/`](deps/bittide-hardware/),
> a real 630-file Clash design traced end to end with every one of its
> dependencies left on its **pristine upstream pin**.
>
> It exists to answer one question honestly: *how much of a real design can
> this plugin trace when you may only touch that design?* The answer, with the
> waveforms to reproduce it and the places it falls short, is in
> **[`deps/README.md`](deps/README.md)** — read that first.
>
> `main` is the plugin alone and builds from a plain clone. The sibling
> branches `dogfood/deps` and `dogfood/shockwaves` extend this one; the diff
> between them is the measure of what each addition buys.
...
`check.sh` builds everything and runs four suites — `manual-instrumentation`
...
entirely by the plugin) and `waveform-tests` (the test suite in two levels:
